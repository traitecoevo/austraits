...
Some studies will have *multiple observations* for some traits, e.g. huber values. This can prevent pivoting from long format to wide. There are two ways to condense multiple observations. The first option `bind_trait_values` concatenates the observations into a single string, thereby retaining all information.
...
data_trait_bound  %>%   
  dplyr::filter(stringr::str_detect(trait_name, "--"))
```
