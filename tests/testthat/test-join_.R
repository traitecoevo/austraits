...
test_that("structure doesn't change", {
  expect_type((join_sites(austraits)), "list")
...
test_that("variables are added", {
 expect_true(ncol(join_sites(austraits)$traits) > ncol(austraits$traits))
...
test_that("are the right variables added?", {
  expect_true(any(names(join_sites(austraits)$traits) == c("latitude (deg)","longitude (deg)")))
