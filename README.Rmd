...
`austraits` allow users to **access, explore and wrangle data** from [traits.build](https://github.com/traitecoevo/traits.build) relational databases. It is also an R interface to the [Australian Plant Trait database](https://austraits.org/) (AusTraits). This package contains functions to join data from various tables, filtering to specific records, combining multiple databases and visualising the distribution of the data. We expect this package will assist users in working with their traits.build databases.
...
For a lightweight installation where dependencies for plotting and the vignettes will not be installed, use:
...
- **For R users**, access and manipulation of the data is assisted with the `austraits` package using `load_austraits()`
...
We highly recommend taking a look at our [Getting Started tutorial](https://traitecoevo.github.io/austraits/) on our website before jumping into the `austraits` R packkage!
...
You can also learn about the definitions of each plant trait with the [AusTraits Plant Dictionary!](https://traitecoevo.github.io/austraits/articles/dictionary.html)
